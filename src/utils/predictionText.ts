...
export const formatPredictionSelection = (
  selection: string,
  t: Translator,
): string => {
  ...
  if (/^both teams to score$/i.test(selection))
    return t("prediction.bothTeamsToScore");
  ...
  // Double Chance: "Team or Draw" → "Team o Pareggio" / "Team A or Team B" → "Team A o Team B"
  const orDrawMatch = selection.match(/^(.+)\s+or Draw$/i);
  if (orDrawMatch) {
    const team = orDrawMatch[1];
    const locale = t("common.or") || "o";
    const drawLabel = t("prediction.draw_short") || "Pareggio";
    return `${team} ${locale} ${drawLabel}`;
  }
  const orTeamMatch = selection.match(/^(.+)\s+or\s+(.+)$/i);
  if (orTeamMatch) {
    const locale = t("common.or") || "o";
    return `${orTeamMatch[1]} ${locale} ${orTeamMatch[2]}`;
  }
...
export const formatReasoningLine = (line: string, t: Translator): string => {
  ...
  if (match)
    return interpolate(t("reason.awayWinStreak"), {
      team: match[1],
      count: match[2],
    });
  ...
  if (match)
    return interpolate(t("reason.eloGap"), { gap: match[1], team: match[2] });

  match = line.match(/^Expected goal total ([\d.]+)/);
  if (match) return interpolate(t("reason.goalsStrong"), { total: match[1] });

  if (line.startsWith("Both attacks averaging > 1 goal"))
    return t("reason.bttsLikely");

  match = line.match(/^(.+) won (\d+) of last (\d+) H2H\.$/);
  if (match)
    return interpolate(t("reason.h2hWins"), {
      team: match[1],
      wins: match[2],
      total: match[3],
    });
